...
- Codex plugin: {codex.get('plugin')} installed={codex.get('plugin_installed')}
- Codex plugin state: {codex.get('plugin_state_file')}
- Codex result mirror: {codex.get('last_result_file')}
- Codex review: {codex.get('review_status')}
- Codex review verdict: {codex.get('review_verdict')}
...
- `codex-plugin-cc` を前提に `/codex:setup`、`/codex:status`、必要時の `/codex:review`、`/codex:result` を実行してください。
- DevOSは `codex-plugin-cc` のstateを `runtime/codex/result.json` へミラーし、STABLE判定の `review OK` に反映します。Codex reviewがunknown/failureの場合はSTABLE扱いにしないでください。
