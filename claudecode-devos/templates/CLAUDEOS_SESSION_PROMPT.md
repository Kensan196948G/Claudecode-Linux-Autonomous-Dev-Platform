...
Codex連携は `codex-plugin-cc`（`codex@openai-codex`）を前提とします。
レビュー実行後は `/codex:status` と `/codex:result` で結果を確認してください。
DevOSハーネスはプラグインstateを `runtime/codex/result.json` へミラーし、STABLE判定の `review OK` に反映します。
...
DevOSは `codex-plugin-cc` のジョブ結果を `runtime/codex/result.json` に取り込みます。
`/codex:result` が未確認、失敗、またはレビュー指摘ありの場合はSTABLE未達として扱ってください。
