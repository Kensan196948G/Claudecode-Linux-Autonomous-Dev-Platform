# 03_ログ監視（LogMonitoring）

## 位置づけ
この文書は `05_監視（Monitoring）` に属する運用ドキュメントです。

## 目的
runtime/logs と agent_logs の確認方法を整理する。
...
## 確認チェックリスト
- [ ] memory-guard.log の設定または運用状態を確認する
- [ ] orchestrator.log の設定または運用状態を確認する
- [ ] repair-ci.log の設定または運用状態を確認する
- [ ] agent_events の設定または運用状態を確認する
