# 01_検証コマンド（ValidationCommands）

## 位置づけ
この文書は `14_テスト（Testing）` に属する運用ドキュメントです。

## 目的
JSON、bash、Python、shellcheck、dashboard API の検証方法をまとめる。
...
## 確認チェックリスト
- [ ] json.tool の設定または運用状態を確認する
- [ ] bash -n の設定または運用状態を確認する
- [ ] py_compile の設定または運用状態を確認する
- [ ] shellcheck の設定または運用状態を確認する
